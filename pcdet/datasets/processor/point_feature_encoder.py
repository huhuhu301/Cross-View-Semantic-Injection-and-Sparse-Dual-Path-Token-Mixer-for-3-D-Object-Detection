    ...
    def forward(self, data_dict):
        ...
        if isinstance(data_dict['points'], list):
            for i in range(len(data_dict['points'])):
                ...
                data_dict['points'][i], use_lead_xyz = getattr(self, self.point_encoding_config.encoding_type)(
                    data_dict['points'][i]
                )
                ...
                if self.point_encoding_config.get('filter_sweeps', False) and 'timestamp' in self.src_feature_list:
                    ...
                    dt = np.round(data_dict['points'][i][:, idx], 2)
                    max_dt = sorted(np.unique(dt))[min(len(np.unique(dt))-1, max_sweeps-1)]
                    data_dict['points'][i] = data_dict['points'][i][dt <= max_dt]
        else:
