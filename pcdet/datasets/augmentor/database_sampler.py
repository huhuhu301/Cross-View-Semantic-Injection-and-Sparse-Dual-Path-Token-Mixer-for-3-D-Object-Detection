...
class DataBaseSampler(object):
    ...
    @staticmethod
    def build_shared_memory_key(database_data_path):
        """Return a run-scoped key when the launcher provides a namespace."""
        namespace = os.environ.get('PCDET_SHARED_MEMORY_NAMESPACE', '').strip()
        if not namespace:
            return database_data_path

        namespace_digest = hashlib.sha256(namespace.encode('utf-8')).hexdigest()[:12]
        return f'{database_data_path}.run_{namespace_digest}'
    ...
    def load_db_to_shared_memory(self):
        ...
        sa_key = self.build_shared_memory_key(self.sampler_cfg.DB_DATA_PATH[0])
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict, mv_height=None, sampled_gt_boxes2d=None):
        ...
        if self.sampler_cfg.get('FILTER_OBJ_POINTS_BY_TIMESTAMP', False) or obj_points.shape[-1] != points[0].shape[-1]:
        ...
        if isinstance(points, list):
            ...
            for i in range(len(points)):
                points_tmp = box_utils.remove_points_in_boxes3d(points[i], large_sampled_gt_boxes)
                points_tmp = np.concatenate([obj_points[:, :points[0].shape[-1]], points_tmp], axis=0)
                num_points_all.append(points_tmp.shape[0])
                points_all.append(points_tmp)
            points = np.concatenate(points_all, axis=0)
            data_dict['num_points_all'] = num_points_all
        else:
    ...
    def __call__(self, data_dict):
        ...
        if total_valid_sampled_dict.__len__() > 0:
            ...
            if isinstance(data_dict['points'], list):
                ...
                num_points_all = []
                for i in range(len(points)):
                    num_points_all.append(points[i].shape[0])
                points = np.concatenate(data_dict['points'], axis=0)
                data_dict['points'] = points
                data_dict['num_points_all'] = num_points_all
