...
class WaymoDataset(DatasetTemplate):
    ...
    def _write_gt_database_global_array(self, all_db_infos, db_data_save_path):
        """Build the optional shared-memory database without holding it in RAM."""
        db_entries = [
            db_info
            for class_infos in all_db_infos.values()
            for db_info in class_infos
        ]
        total_points = sum(int(x['num_points_in_gt']) for x in db_entries)

        num_point_features = None
        for db_info in db_entries:
            num_points = int(db_info['num_points_in_gt'])
            if num_points == 0:
                continue
            point_file = self.root_path / db_info['path']
            flat_points = np.fromfile(str(point_file), dtype=np.float32)
            if flat_points.size % num_points != 0:
                raise ValueError('Invalid GT database point file: %s' % point_file)
            num_point_features = flat_points.size // num_points
            break

        db_data_save_path.parent.mkdir(parents=True, exist_ok=True)
        if num_point_features is None:
            np.save(str(db_data_save_path), np.empty((0, 0), dtype=np.float32))
            for db_info in db_entries:
                db_info['global_data_offset'] = [0, 0]
            return 0

        global_points = np.lib.format.open_memmap(
            str(db_data_save_path), mode='w+', dtype=np.float32,
            shape=(total_points, num_point_features)
        )
        point_offset = 0
        for db_info in db_entries:
            point_file = self.root_path / db_info['path']
            points = np.fromfile(str(point_file), dtype=np.float32)
            if points.size % num_point_features != 0:
                raise ValueError('Inconsistent GT database point width: %s' % point_file)
            points = points.reshape(-1, num_point_features)
            expected_points = int(db_info['num_points_in_gt'])
            if points.shape[0] != expected_points:
                raise ValueError(
                    'GT database point count mismatch for %s: expected %d, got %d'
                    % (point_file, expected_points, points.shape[0])
                )
            end_offset = point_offset + expected_points
            global_points[point_offset:end_offset] = points
            db_info['global_data_offset'] = [point_offset, end_offset]
    ...
    def create_groundtruth_database(self, info_path, save_path, used_classes=None, split='train', sampled_interval=10,
        ...
        for k in tqdm(range(0, len(infos), sampled_interval)):
            ...
            for i in range(num_obj):
                ...
                if (used_classes is None) or names[i] in used_classes:
                    ...
                    db_path = os.path.relpath(filepath, self.root_path)
        ...
        point_offset_cnt = self._write_gt_database_global_array(all_db_infos, db_data_save_path)
        print('Shared GT database is saved to %s (%d points)' % (db_data_save_path, point_offset_cnt))
    ...
    def create_groundtruth_database_parallel(self, info_path, save_path, used_classes=None, split='train', sampled_interval=10,
        ...
        if use_sequence_data:
            ...
            db_data_save_path = save_path / ('%s_gt_database_%s_sampled_%d_multiframe_%s_to_%s_%sparallel_global.npy' % (processed_data_tag, split, sampled_interval, st_frame, ed_frame, 'tail_' if crop_gt_with_tail else ''))
        else:
            ...
            db_data_save_path = save_path / ('%s_gt_database_%s_sampled_%d_parallel_global.npy' % (processed_data_tag, split, sampled_interval))
...
def create_waymo_infos(dataset_cfg, class_names, data_path, save_path,
    ...
    if data_path != save_path:
        raise ValueError(
            'Waymo data_path and save_path must be identical so processed data, '
            'infos, and relative GT sampler paths share one root: %s != %s'
            % (data_path, save_path)
        )
...
def create_waymo_gt_database(
    ...
    workers=min(16, multiprocessing.cpu_count()), use_parallel=False, crop_gt_with_tail=False, split='train'):
    data_path = Path(data_path).expanduser().resolve()
    save_path = Path(save_path).expanduser().resolve()
    if data_path != save_path:
        raise ValueError(
    ...
    filename = save_path / ('%s_infos_%s.pkl' % (processed_data_tag, split))
    ...
    if use_parallel:
        dataset.create_groundtruth_database_parallel(
            info_path=filename, save_path=save_path, split=split, sampled_interval=1,
            used_classes=['Vehicle', 'Pedestrian', 'Cyclist'], processed_data_tag=processed_data_tag,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg_file', type=str, required=True, help='dataset YAML configuration')
    parser.add_argument('--func', type=str, default='create_waymo_gt_database', help='data preparation function')
    parser.add_argument('--data_path', type=str, default=None, help='Waymo dataset root (overrides DATA_PATH)')
    parser.add_argument('--save_path', type=str, default=None, help='output root (defaults to data_path)')
    parser.add_argument('--split', type=str, default='train', help='info split used to build the GT database')
    parser.add_argument('--workers', type=int, default=min(16, multiprocessing.cpu_count()), help='worker processes')
    parser.add_argument('--processed_data_tag', type=str, default='waymo_processed_data_v0_5_0', help='processed data directory prefix')
    parser.add_argument('--update_info_only', action='store_true', default=False, help='only refresh info files')
    parser.add_argument('--use_parallel', action='store_true', default=False, help='build the GT database with CPU workers')
    parser.add_argument('--wo_crop_gt_with_tail', action='store_true', default=False, help='disable multiframe tail cropping')

    args = parser.parse_args()
    if args.save_path is not None:
        data_root = Path(args.data_path).expanduser().resolve() if args.data_path else None
        save_root = Path(args.save_path).expanduser().resolve()
        if data_root is None:
            parser.error('--data_path is required when --save_path is provided')
        if data_root != save_root:
            parser.error(
                '--data_path and --save_path must be identical; processed point files, '
                'infos, and GT sampler paths share the Waymo dataset root'
            )
    ...
    if args.func == 'create_waymo_infos':
        ...
        data_path = Path(args.data_path or dataset_cfg.DATA_PATH).expanduser().resolve()
        save_path = Path(args.save_path).expanduser().resolve() if args.save_path else data_path
        create_waymo_infos(
