...
def decode_bbox_from_sparse_heatmap(
    ...
    angle = torch.atan2(rot_sin, rot_cos)
    xs = (spatial_indices[:, :, -1:] + center[:, :, 0:1] + 0.5) * feature_map_stride * voxel_size[0] + point_cloud_range[0]
    ys = (spatial_indices[:, :, -2:-1] + center[:, :, 1:2] + 0.5) * feature_map_stride * voxel_size[1] + point_cloud_range[1]
...
def decode_bbox(
    ...
    angle = torch.atan2(rot_sin, rot_cos)
    xs = center[:, :, 0:1] * feature_map_stride * voxel_size[0] + point_cloud_range[0]
    ys = center[:, :, 1:2] * feature_map_stride * voxel_size[1] + point_cloud_range[1]
