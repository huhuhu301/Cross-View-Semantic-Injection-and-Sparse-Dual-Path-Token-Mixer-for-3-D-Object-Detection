...
class RangeViewVFE(VFETemplate):
    ...
    def __init__(self, model_cfg, num_point_features, voxel_size, grid_size, point_cloud_range, **kwargs):
        ...
        rv_feature_dim = model_cfg.RV_FEATURE_DIM
        ...
        self.scale_yz = grid_size[1] * grid_size[2]
        self.scale_z = grid_size[2]
        # Keep geometry constants device-agnostic at construction time.  The
        # non-persistent buffers follow model.to(device) without adding keys to
        # the historical checkpoint schema.
        self.register_buffer(
        ...
        self.rv_token_only = model_cfg.get('RV_TOKEN_ONLY', False)
    ...
    def forward(self, batch_dict, **kwargs):
        ...
        # Build RV features and the point-to-pixel mapping.
        rv_features, linear_indices, pixel_counts, u_float, v_float = \
            self.create_range_view_feature(points.clone(), *self.rv_feature_shape, self.rv_feature_fov)  # [B,6,H,W], [N], [B*H*W], [N], [N]

        rv_features = self.rv_backbone(self.rv_input_roj(rv_features))  # [B, C_rv, H, W]
        B, C, H, W = rv_features.shape

        if self.rv_enabled:
            pixel_counts = pixel_counts.reshape(B, H * W)               # [B, H*W]
            non_zero_mask = (pixel_counts > 0)                          # [B, H*W]
            rv_tokens = rv_features.flatten(2).permute(0, 2, 1).contiguous()  # [B, H*W, C]

            rv_tokens_list, rv_uv_list = [], []
            # Construct (u, v) coordinates in the same order as flatten().
            u_grid = torch.arange(W, device=rv_features.device).repeat(H)                  # [H*W]
            v_grid = torch.arange(H, device=rv_features.device).unsqueeze(1).repeat(1, W).reshape(-1)  # [H*W]

            for b in range(B):
                mask_b = non_zero_mask[b]                                     # [H*W]
                rv_tokens_list.append(rv_tokens[b][mask_b])                   # (N_r_b, C)

                uv_b = torch.stack([u_grid[mask_b], v_grid[mask_b]], dim=1)   # (N_r_b, 2)
                uv_b = uv_b / uv_b.new_tensor([W - 1, H - 1])
                rv_uv_list.append(uv_b)

            batch_dict['rv_tokens'] = rv_tokens_list
            batch_dict['rv_uv'] = rv_uv_list

        # Voxel branch.
        if not self.rv_token_only:
            # Gather RV features at point locations.
            rv_feat_2d = rv_features.permute(0, 2, 3, 1).reshape(-1, C)       # [(B*H*W), C]
            point_features = rv_feat_2d[linear_indices]                       # [N, C]

            # Deduplicate point voxel coordinates and aggregate point features.
            points_coords = torch.floor((points[:, 1:4] - self.point_cloud_range[None, :3]) / self.voxel_size[None, :3]).int()
            mask = ((points_coords >= 0) & (points_coords < self.grid_size[None, :3])).all(dim=1)

            points, points_coords = points[mask], points_coords[mask]
            point_features = point_features[mask]
            u_f, v_f = u_float[mask], v_float[mask]                            # [N_mask]

            merge_coords = points[:, 0].int() * self.scale_xyz + \
                points_coords[:, 0] * self.scale_yz + points_coords[:, 1] * self.scale_z + points_coords[:, 2]

            unq_coords, unq_inv = torch.unique(merge_coords, return_inverse=True, dim=0)  # [N_vox], [N_mask]

            # Point geometry features.
            points_xyz = points[:, 1:4]
            points_mean = scatter_mean(points_xyz, unq_inv, dim=0)             # [N_vox, 3]
            f_cluster = points_xyz - points_mean[unq_inv, :]
            f_center = points_xyz - (points_coords.to(points_xyz.dtype) * self.voxel_size[None, :3] + self.offset[None, :3])

            # Mean normalized RV coordinate for every voxel.
            uv_pf = torch.stack([u_f / (W - 1), v_f / (H - 1)], dim=1)         # [N_mask, 2]
            vox_uv = scatter_mean(uv_pf, unq_inv, dim=0)                       # [N_vox, 2]
            batch_dict['vox_uv'] = vox_uv

            # Pillar/voxel features.
            features = torch.cat([points[:, 1:], f_cluster, f_center, point_features], dim=-1)
            for pfn in self.pfn_layers:
                features = pfn(features, unq_inv)

            # voxel_coords: [N_vox, 4] (b, z, y, x)
            unq_coords = unq_coords.int()
            ...
            # Token-only ablation does not expose voxel/RV anchors.
            dummy_features = torch.stack([
                t.mean(0) if t.numel() > 0 else torch.zeros(C, device=points.device)
                for t in batch_dict['rv_tokens']
            ])
            batch_dict['pillar_features'] = batch_dict['voxel_features'] = dummy_features
            batch_dict['voxel_coords'] = torch.zeros((B, 4), dtype=torch.int, device=points.device)

        return batch_dict


    def create_range_view_feature(self, points, H=128, W=1024, fov_vertical=180):
        device = points.device
        batch_idx = points[:, 0].long()
        xyz = points[:, 1:4]
        r = torch.norm(xyz, dim=1)                                   # [N]
        phi = torch.atan2(xyz[:, 1], xyz[:, 0])                      # [-pi, pi]
        theta = torch.asin(xyz[:, 2] / r.clamp(min=1e-6))
