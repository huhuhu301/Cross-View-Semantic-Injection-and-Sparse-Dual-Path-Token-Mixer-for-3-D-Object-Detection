...
class RouterLinearAttention(nn.Module):
    ...
    def forward(self, feats, batch_ids, batch_size):
        ...
        # Dense AV2 frames can produce rare FP16 overflows anywhere along the
        # global-attention path. Keep the complete attention island in FP32,
        # including projections, reductions, and the output projection. This
        # changes no parameters or routing decisions; restoring the input dtype
        # at the boundary preserves the surrounding sparse-block contract.
        with torch.cuda.amp.autocast(enabled=False):
            ...
            # Gather the per-sample global aggregates once and evaluate all
            # router tokens in a batch. This is algebraically identical to the
            # former token-wise loop but avoids one kernel launch per token.
...
class RouterBranch(spconv.SparseModule):
    ...
    def forward(self, x):
        ...
        if self.keep_ratio < 1.0:
            ...
            if (
                self.score_straight_through
                and self.selection_strategy in ['learned', 'learned_hybrid']
                and self.score_head is not None
            ):
                # Top-k indices are discrete, so the score head otherwise gets
                # no gradient. This factor is exactly one in the forward pass
                # and supplies a straight-through gradient in the backward pass.
                score_logits = self.score_head(features).squeeze(-1)
                if self.score_temperature != 1.0:
                    score_logits = score_logits / self.score_temperature
                score_prob = torch.sigmoid(score_logits)
                score_scale = 1.0 + score_prob - score_prob.detach()
                features = features * score_scale.unsqueeze(-1)
        else:
            sub_indices = indices
    ...
    def _select_indices(self, features, candidate_idx, num_keep):
        ...
        if self.selection_strategy in ['importance', 'learned']:
...
class SDTMBlock(spconv.SparseModule):
    ...
    def __init__(self, dim, router_dim, stride_xy, attn_heads, attn_dim, indice_tag,
        ...
        super().__init__()
        self.local_mixer = LocalMixer(dim, indice_tag)
    ...
    def forward(self, x):
        ...
        if self.use_router:
            ...
            gate = self.router_gate(residual.features)
...
class AttnPillarPool(nn.Module):
    ...
    def forward(self, x):
        ...
        # AMP can overflow the QK score product on dense AV2 pillars even
        # though every input feature is finite. Keep the architecture and
        # gradients unchanged, but evaluate this numerically sensitive
        # attention/residual island in FP32. The original LayerNorm already
        # promoted this module's output to FP32 under autocast, so the output
        # dtype contract is unchanged.
        with torch.cuda.amp.autocast(enabled=False):
...
class RVSDTM(nn.Module):
    ...
    def __init__(self, model_cfg, input_channels, grid_size, class_names, voxel_size, point_cloud_range, **kwargs):
        ...
        # === RV-guided cross-view fusion (optional) ===
        self.use_rv = model_cfg.get('USE_RV', False)
        if self.use_rv:
            rv_feat_dim = model_cfg.get('RV_FEATURE_DIM', None)
            if rv_feat_dim is None:
                raise ValueError("USE_RV=True requires BACKBONE_3D.RV_FEATURE_DIM to match VFE.RV_FEATURE_DIM.")
            rv_attn_dim = model_cfg.get('RV_ATTN_DIM', dim)
            rv_heads = model_cfg.get('RV_HEADS', 8)
            rv_k = model_cfg.get('RV_DCA_K', 16)
            rv_max_tokens = model_cfg.get('RV_MAX_TOKENS', 12000)
            rv_vox_chunk = model_cfg.get('RV_VOX_CHUNK', 2048)
            rv_su = model_cfg.get('RV_SU', 0.05)
            rv_sv = model_cfg.get('RV_SV', 0.08)
            rv_sigma = model_cfg.get('RV_SIGMA', 1.0)
            rv_tau = model_cfg.get('RV_TAU', 1.0)
            rv_alpha_init = model_cfg.get('RV_ALPHA_INIT', 0.3)
            self.rv_multi_scale = model_cfg.get('RV_MULTI_SCALE', False)
            self.rv_near_range = model_cfg.get('RV_DCA_NEAR_RANGE', 40.0)

            self.rv_projection = nn.Linear(rv_feat_dim, rv_attn_dim, bias=False) if rv_feat_dim != rv_attn_dim else nn.Identity()
            self.cross_view_dca = CrossViewDCA(
                ...
                heads=rv_heads,
                k=rv_k,
                max_rv_tokens=rv_max_tokens,
                vox_chunk=rv_vox_chunk,
                su=rv_su,
                sv=rv_sv,
                sigma=rv_sigma,
                tau=rv_tau,
                alpha_init=rv_alpha_init
            )
            # mid-stage lightweight DCA (optional, smaller K/token cap, with voxel selection)
            self.use_rv_mid = model_cfg.get('RV_MID_DCA_ENABLE', False)
            if self.use_rv_mid:
                mid_attn_dim = model_cfg.get('RV_MID_DCA_ATTN_DIM', rv_attn_dim)
                mid_heads = model_cfg.get('RV_MID_DCA_HEADS', rv_heads)
                mid_k = model_cfg.get('RV_MID_DCA_K', 4)
                mid_max_tokens = model_cfg.get('RV_MID_DCA_MAX_TOKENS', 6000)
                mid_vox_chunk = model_cfg.get('RV_MID_DCA_VOX_CHUNK', 1024)
                mid_su = model_cfg.get('RV_MID_DCA_SU', rv_su)
                mid_sv = model_cfg.get('RV_MID_DCA_SV', rv_sv)
                mid_sigma = model_cfg.get('RV_MID_DCA_SIGMA', rv_sigma)
                mid_tau = model_cfg.get('RV_MID_DCA_TAU', rv_tau)
                mid_alpha = model_cfg.get('RV_MID_DCA_ALPHA_INIT', 0.25)
                self.rv_mid_keep_ratio = model_cfg.get('RV_MID_DCA_KEEP_RATIO', 0.3)
                self.rv_mid_min_tokens = model_cfg.get('RV_MID_DCA_MIN_TOKENS', 2)
                self.cross_view_dca_mid = CrossViewDCA(
                    vox_c=dim,
                    rv_c=mid_attn_dim,
                    attn_dim=mid_attn_dim,
                    heads=mid_heads,
                    k=mid_k,
                    max_rv_tokens=mid_max_tokens,
                    vox_chunk=mid_vox_chunk,
                    su=mid_su,
                    sv=mid_sv,
                    sigma=mid_sigma,
                    tau=mid_tau,
                    alpha_init=mid_alpha
                )
                # reuse rv_projection; if mid_attn_dim differs, add projection
                if rv_feat_dim != mid_attn_dim:
                    self.rv_projection_mid = nn.Linear(rv_feat_dim, mid_attn_dim, bias=False)
                else:
                    self.rv_projection_mid = self.rv_projection
            else:
                self.use_rv_mid = False
    ...
    def rv_fusion(self, x, batch_dict):
        if not self.use_rv:
            return x
        if ('rv_tokens' not in batch_dict) or ('rv_uv' not in batch_dict) or ('vox_uv' not in batch_dict):
            return x
        vox_uv_all = batch_dict['vox_uv']
        if vox_uv_all.shape[0] != x.features.shape[0]:
            return x

        # Optional multi-scale RV fusion; otherwise use the high-resolution tokens.
        if self.rv_multi_scale:
            rv_tokens_hi = batch_dict.get('rv_tokens_hi', batch_dict['rv_tokens'])
            rv_uv_hi = batch_dict.get('rv_uv_hi', batch_dict['rv_uv'])
            rv_tokens_lo = batch_dict.get('rv_tokens_lo', rv_tokens_hi)
            rv_uv_lo = batch_dict.get('rv_uv_lo', rv_uv_hi)

            rv_tokens_hi = [self.rv_projection(t) for t in rv_tokens_hi]
            rv_tokens_lo = [self.rv_projection(t) for t in rv_tokens_lo]

            voxel_idx = x.indices[:, [3, 2]].float()  # x_idx, y_idx
            center_xy = voxel_idx
            center_xy[:, 0] = (voxel_idx[:, 0] + 0.5) * self.voxel_size[0] + self.point_cloud_range[0]
            center_xy[:, 1] = (voxel_idx[:, 1] + 0.5) * self.voxel_size[1] + self.point_cloud_range[1]
            ranges = torch.sqrt(center_xy[:, 0] ** 2 + center_xy[:, 1] ** 2)
            near_mask = ranges <= self.rv_near_range
            ...
            for b in range(x.batch_size):
                ...
                if not batch_mask.any():
                    continue
                if rv_tokens_hi[b].numel() > 0:
                    mask_near = batch_mask & near_mask
                    if mask_near.any():
                        features[mask_near] = self.cross_view_dca(
                            features[mask_near],
                            vox_uv_all[mask_near],
                            rv_tokens_hi[b],
                            rv_uv_hi[b]
                        )
                if rv_tokens_lo[b].numel() > 0:
                    mask_far = batch_mask & (~near_mask)
                    if mask_far.any():
                        features[mask_far] = self.cross_view_dca(
                            features[mask_far],
                            vox_uv_all[mask_far],
                            rv_tokens_lo[b],
                            rv_uv_lo[b]
        ...
        # Single-scale path.
        rv_tokens = [self.rv_projection(t) for t in batch_dict['rv_tokens']]
        rv_uv = batch_dict['rv_uv']
        features = x.features.clone()
        for b in range(x.batch_size):
            mask = x.indices[:, 0] == b
            if mask.any() and rv_tokens[b].numel() > 0:
                features[mask] = self.cross_view_dca(
                    features[mask],
                    vox_uv_all[mask],
                    rv_tokens[b],
                    rv_uv[b]
    ...
    def _mid_dca_select_mask(self, x):
        if (not self.use_rv_mid) or self.rv_mid_keep_ratio >= 1.0:
            return None
        features = x.features
        indices = x.indices
        batch_ids = indices[:, 0]
        ...
        for b in range(x.batch_size):
            ...
            num = max(self.rv_mid_min_tokens, int(math.ceil(b_idx.numel() * self.rv_mid_keep_ratio)))
            feats_b = features[b_mask]
            scores = feats_b.pow(2).sum(dim=1)
            topk = torch.topk(scores, k=min(num, scores.shape[0]), dim=0).indices
            keep_mask[b_idx[topk]] = True
        return keep_mask

    def rv_mid_dca(self, x, batch_dict):
        if not (self.use_rv and self.use_rv_mid):
            return x
        if ('rv_tokens' not in batch_dict) or ('rv_uv' not in batch_dict) or ('vox_uv' not in batch_dict):
            return x
        vox_uv_all = batch_dict['vox_uv']
        ...
        rv_tokens_list = [self.rv_projection_mid(t) for t in batch_dict['rv_tokens']]
        rv_uv_list = batch_dict['rv_uv']

        keep_mask = self._mid_dca_select_mask(x)
        if keep_mask is None:
            # use all voxels
            features = x.features.clone()
            for b in range(x.batch_size):
                mask = x.indices[:, 0] == b
                if mask.any() and rv_tokens_list[b].numel() > 0:
                    features[mask] = self.cross_view_dca_mid(
                        features[mask],
                        vox_uv_all[mask],
                        rv_tokens_list[b],
                        rv_uv_list[b]
        ...
        # selected voxels only
        features = x.features.clone()
        for b in range(x.batch_size):
            mask_b = (x.indices[:, 0] == b) & keep_mask
            if mask_b.any() and rv_tokens_list[b].numel() > 0:
                features[mask_b] = self.cross_view_dca_mid(
                    features[mask_b],
                    vox_uv_all[mask_b],
                    rv_tokens_list[b],
                    rv_uv_list[b]
    ...
    def forward(self, batch_dict):
        ...
        x = self.cpe(x)
        x = self.stage1(x)
        x = self.rv_mid_dca(x, batch_dict)
        x = self.stage2(x)
