...
class SparseDynamicHead(nn.Module):
    ...
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range, voxel_size,
        ...
        # Geometry constants follow model placement without extending the
        ...
        self.dynamic_pos_num = self._normalize_per_head_value(
            'DYNAMIC_POS_NUM', default=5)
        self.regression_layout = self.model_cfg.get(
            'REGRESSION_LAYOUT', 'packed_bbox')
        self._validate_regression_layout()
    ...
    def _normalize_per_head_value(self, key, default):
        value = self.model_cfg.get(key, default)
        if isinstance(value, (list, tuple)):
            if len(value) != len(self.class_names_each_head):
                raise ValueError(
                    f'{key} must contain one value per detection head: '
                    f'{len(value)} values for '
                    f'{len(self.class_names_each_head)} heads'
                )
            values = [int(item) for item in value]
        else:
            values = [int(value)] * len(self.class_names_each_head)
        if any(item <= 0 for item in values):
            raise ValueError(f'{key} values must all be positive: {values}')
        return values

    def _validate_regression_layout(self):
        head_order = list(self.separate_head_cfg.HEAD_ORDER)
        head_dict = self.separate_head_cfg.HEAD_DICT
        layouts = {
            'packed_bbox': ['bbox'],
            'split_fields': ['center', 'center_z', 'dim', 'rot'],
        }
        if self.regression_layout not in layouts:
            raise ValueError(
                'REGRESSION_LAYOUT must be packed_bbox or split_fields, got '
                f'{self.regression_layout}'
            )
        required = layouts[self.regression_layout]
        if head_order != required:
            raise ValueError(
                f'{self.regression_layout} requires HEAD_ORDER={required}, '
                f'got {head_order}'
            )
        ...
        for name in head_order:
            if name not in head_dict:
                raise ValueError(
                    f'HEAD_ORDER field {name} is absent from HEAD_DICT')
            if name in expected_channels:
                channels = int(head_dict[name]['out_channels'])
                if channels != expected_channels[name]:
                    raise ValueError(
                        f'{name} must have {expected_channels[name]} channels, '
                        f'got {channels}'
                    )
        self.regression_code_size = sum(
            int(head_dict[name]['out_channels']) for name in head_order)
        if self.regression_code_size != 8:
            raise ValueError(
                'SparseDynamicHead expects exactly 8 box channels, got '
                f'{self.regression_code_size}'
            )
    ...
    def assign_target_of_single_head(
            self, num_classes, gt_boxes, feature_map_size,
            feature_map_stride, spatial_indices, dynamic_pos_num,
            num_max_objs=500):
        """
        Args:
            gt_boxes: (N, 8)
        ...
        ret_boxes = gt_boxes.new_zeros(
            (num_max_objs, dynamic_pos_num, self.regression_code_size))
        inds = gt_boxes.new_zeros(num_max_objs, dynamic_pos_num).long()
        mask = gt_boxes.new_zeros(num_max_objs, dynamic_pos_num).long()
        center_distances = gt_boxes.new_zeros(num_max_objs, dynamic_pos_num)
        ...
        gt_boxes = gt_boxes[box_masks][:num_max_objs]
        box_num = len(gt_boxes)
        if box_num and len(spatial_indices) < dynamic_pos_num:
            raise ValueError(
                f'DYNAMIC_POS_NUM={dynamic_pos_num} exceeds the '
                f'{len(spatial_indices)} active BEV sites'
            )
        ...
        sort_distance = sort_res[0][:, :dynamic_pos_num]
        sort_inds = sort_res[1][:, :dynamic_pos_num]
            ...
            for bs_idx in range(batch_size):
                ...
                for box_idx, name in enumerate(gt_class_names):
                    if name not in cur_class_names:
                        continue
                    temp_box = cur_gt_boxes[box_idx].clone()
                ...
                heatmap, ret_boxes, inds, mask, distances = self.assign_target_of_single_head(
                    ...
                    dynamic_pos_num=self.dynamic_pos_num[head_idx],
    ...
    def get_loss(self):
        ...
        for idx, pred_dict in enumerate(pred_dicts):
            ...
            pred_reg[..., 6:8] = pred_reg[..., 6:8].sigmoid() * 2 - 1
    ...
    def get_iou_targets(self, box_preds, box_targets, masks, spatial_indices):
        ...
        gboxes = self._get_predicted_boxes(
            box_targets[box_inds], spatial_indices[box_inds])
    ...
    def generate_predicted_boxes(self, batch_size, pred_dicts, sparse_indices, post_process_cfg):
        post_center_limit_range = self.point_cloud_range.new_tensor(
            post_process_cfg.POST_CENTER_LIMIT_RANGE)
        ...
        for idx, pred_dict in enumerate(pred_dicts):
            ...
            for bidx, final_dict in enumerate(final_pred_dicts):
                class_mapping = self.class_id_mapping_each_head[idx].to(
                    final_dict['pred_labels'].device)
                final_dict['pred_labels'] = class_mapping[
    ...
    def forward(self, data_dict):
        ...
        if self.training:
            ...
            if self.regression_layout == 'packed_bbox':
                data_dict['final_box_dicts'] = self.generate_predicted_boxes_v2(
                    data_dict['batch_size'], pred_dicts, spatial_indices)
            else:
                data_dict['final_box_dicts'] = self.generate_predicted_boxes(
                    data_dict['batch_size'], pred_dicts, spatial_indices,
                    self.model_cfg.POST_PROCESSING)
