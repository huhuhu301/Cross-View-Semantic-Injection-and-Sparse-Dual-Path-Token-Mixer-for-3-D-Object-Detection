...
def merge_new_config(config, new_config, _base_stack=()):
    if '_BASE_CONFIG_' in new_config:
        base_path = Path(new_config['_BASE_CONFIG_']).resolve()
        if base_path in _base_stack:
            raise ValueError('Cyclic configuration inheritance: {}'.format(base_path))
        with base_path.open('r') as f:
            yaml_config = yaml.safe_load(f)
        merge_new_config(config, yaml_config, _base_stack + (base_path,))
    ...
    for key, val in new_config.items():
        ...
        if key not in config:
            config[key] = EasyDict()
        merge_new_config(config[key], val, _base_stack)
