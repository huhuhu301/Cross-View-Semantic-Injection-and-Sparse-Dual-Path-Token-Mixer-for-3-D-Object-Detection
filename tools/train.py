...
def parse_config():
    ...
    parser.add_argument('--cfg_file', type=str, required=True, help='training YAML')

    parser.add_argument('--batch_size', type=int, default=None, required=False, help='global batch size; defaults to YAML batch per GPU')
    ...
    parser.add_argument('--seed', type=int, default=None, help='base training seed; overrides YAML SEED')
    ...
    parser.add_argument('--skip_post_eval', action='store_true',
                        help='skip train.py post-training evaluation (run tools/test.py separately)')
    ...
    args.sync_bn = args.sync_bn or cfg.get('SYNC_BN', False)
    seed = args.seed if args.seed is not None else cfg.get('SEED')
    args.fix_random_seed = args.fix_random_seed or seed is not None
    args.seed = int(seed) if seed is not None else 666
...
def main():
    ...
    if cfg.LOCAL_RANK == 0:
        shutil.copy2(args.cfg_file, output_dir / Path(args.cfg_file).name)
    ...
    if dist_train:
        model = nn.parallel.DistributedDataParallel(
            ...
            # Stochastic router choices can leave a branch unused on a step.
    ...
    if args.skip_post_eval:
        logger.info('Post-training evaluation skipped; evaluate saved checkpoints with tools/test.py.')
        return
