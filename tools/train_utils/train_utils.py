...
def _global_loss_is_finite(loss):
    """Coordinate non-finite loss detection across all distributed ranks."""
    local_bad = (~torch.isfinite(loss.detach())).any()
    global_bad = local_bad.to(dtype=torch.int32)
    if torch.distributed.is_available() and torch.distributed.is_initialized():
        torch.distributed.all_reduce(
            global_bad, op=torch.distributed.ReduceOp.MAX
        )
    return not bool(global_bad.item()), bool(local_bad.item())
...
def train_one_epoch(model, optimizer, train_loader, model_func, lr_scheduler, accumulated_iter, optim_cfg,
    ...
    if total_it_each_epoch == len(train_loader):
        ...
        if start_it:
            if rank == 0 and logger is not None:
                logger.info(
                    'Fast-forwarding DataLoader by %d batches for a '
                    'mid-epoch resume', start_it
                )
            for _ in range(start_it):
                try:
                    ...
                    raise RuntimeError(
                        'DataLoader ended while restoring the mid-epoch '
                        f'cursor at batch {start_it}'
                    ) from error
    ...
    if rank == 0:
        ...
        batch_loss = common_utils.AverageMeter()
    ...
    for cur_it in range(start_it, total_it_each_epoch):
        ...
        batch['total_epoch'] = total_epochs
        with torch.cuda.amp.autocast(enabled=use_amp):
            loss, tb_dict, disp_dict = model_func(model, batch)

        loss_is_finite, local_loss_is_bad = _global_loss_is_finite(loss)
        if not loss_is_finite:
            raise FloatingPointError(
                'globally coordinated non-finite forward loss at '
                f'accumulated_iter={accumulated_iter}; '
                f'rank={rank}, local_loss_is_bad={local_loss_is_bad}'
            )
        ...
        avg_batch_loss = commu_utils.average_reduce_value(loss.item())
        ...
        if rank == 0:
            ...
            batch_loss.update(avg_batch_loss)

            disp_dict.update({
                'loss': f'{batch_loss.val:.3f}({batch_loss.avg:.3f})', 'lr': cur_lr, 'd_time': f'{data_time.val:.2f}({data_time.avg:.2f})',
    ...
    if rank == 0:
        pbar.close()
        return accumulated_iter, batch_loss.avg
    else:
...
def train_model(model, optimizer, train_loader, model_func, lr_scheduler, optim_cfg,
    ...
    with tqdm.trange(start_epoch, total_epochs, desc='epochs', dynamic_ncols=True, leave=(rank == 0)) as tbar:
        ...
        for cur_epoch in tbar:
            ...
            accumulated_iter, batch_loss = train_one_epoch(
            ...
            if rank == 0:
                logger.info("epoch{}_losses: {}".format(cur_epoch, batch_loss))
        ...
        logger.info("batch_losses: {}".format(batch_losses))
